...
El destino elegido es Render. El nombre de servicio propuesto es `pokemon-quiz-arcade`, por lo que Render intentará asignar `https://pokemon-quiz-arcade.onrender.com`. La disponibilidad exacta del subdominio se confirma al crear el servicio; si Render asigna otro, cambia `PUBLIC_BASE_URL` por la URL real.
...
3. Usa Build Command `pip install -r requirements.txt` y Start Command `uvicorn app:app --host 0.0.0.0 --port $PORT`.
